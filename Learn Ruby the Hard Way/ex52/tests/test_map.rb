...
class TestGame < Test::Unit::TestCase
  ...
  def test_room()
    ...
    assert_equal("GoldRoom", gold.name) # check's the classes name was initialized
    assert_equal({}, gold.paths) # checks's empty paths hash was created
  end
  ...
  def test_room_paths()
    ...
    assert_equal(north, center.go('north')) # go method returns proper class instance from paths
  ...
  def test_map()
    ...
    assert_equal(west, start.go('west')) # head west
    assert_equal(start, start.go('west').go('east')) # return to start via west
    assert_equal(start, start.go('down').go('up')) # return to start via down
  end
  ...
  def test_gothon_game_map()
    ...
    # TODO: Test should play through the whole game
  end
